...
- 永続化には、ext4でフォーマットしラベルを `persistence` にした
  ...
  `/home` の1行だけ) が必要です。
- 暗号化 (LUKS等) は未実装です。
- GUIによるパーティション作成はソース上で実装済みです (詳細は
  「GUIによる永続領域作成 (実装仕様)」節を参照)。ただし、この変更を含む
  ISOはまだ再ビルドしていないため、Live環境でのGUI表示・操作・実ディスク
  作成はいずれも未検証です。
...
- GUIによる永続領域作成はソース上で実装済みですが、本手順 (GUIを介さず
  `parted`/`mkfs.ext4`/`mount`/`tee`を直接実行するBIOS/UEFI手動永続化
  テスト) の対象には含まれていません。この変更を含むISOはまだ
  再ビルドしていないため、Live環境でのGUI表示・操作、およびGUI経由の
  実ディスク作成は別途未検証です (詳細は「GUIによる永続領域作成
  (実装仕様)」節を参照)。
- LUKSによる暗号化は未実装です。
...
確認済みです。この変更を含むISOのビルド、Live環境でのGUI実動作、GUI
経由の実ディスク作成、暗号化、アプリ本体の永続化といった、本手順の
対象外または未実装の機能については、確認済みとはみなしていません。
...
**一方、この変更を含むISOはまだ再ビルドしていない。** そのため、次の
項目はいずれも未検証である。

- 再ビルドしたISOでの、この実装を含む成果物としての動作。
- 実際のLive環境上でGUIを表示し、実際に操作すること (GUI実動作)。
- GUI経由での実ディスクに対する実際のパーティション作成・フォーマット
  (実sudo・実helper・実parted/mkfs/mountの実行)。

これらはいずれも本節で示す静的検査・モックテストの対象外であり、上記
「手動テスト手順」節のBIOS/UEFI手動永続化テスト (GUIを介さず`parted`/
`mkfs.ext4`/`mount`/`tee`を直接実行して確認したもの) とも別の検証範囲
である。両者を混同しない。以下は、この実装が従う仕様をまとめたものである。
