...
**現時点では設計のみであり、未実装です。** 上記「手動テスト手順」で確認済みの
操作 (パーティション作成・ext4フォーマット・`persistence.conf`作成) を、
GUIから安全に実行できるようにするための設計方針を以下にまとめる。
...
- `MOUNTPOINTS` が空でない、または子パーティションのいずれかがマウント
  されているデバイス。
- スワップとして使用中のデバイス (`FSTYPE=swap` または `swapon --show` に
  現れるもの)。
...
- Live起動元のデバイス。`/run/live/medium` の `SOURCE`
  ...
  device mapperを介している場合も `lsblk` の親子関係
  (`PKNAME`/`--inverse`、および `/sys/class/block/*/slaves/`) をたどって
  最終的な親diskを特定し、除外する。overlayの `/` (upperdir) の
  デバイスだけを見て判定しない。BIOS/UEFI問わず同じロジックを用いる。
...
- 本READMEの「手動テスト手順」により、**現在のMyPocketOS Liveイメージに
  ...
  実機確認済み**である。初版はこの実機確認済みの事実を根拠に`sudo`を
  用いる。live-configのどの設定ファイル・仕組みによってNOPASSWDが
  付与されているかは今回直接確認していないため、その特定の仕組みを
  断定しない。実装時には、`sudo -n`によって都度その時点での利用可否を
...
- GUI本体 (`/usr/local/bin/mypocketos-persistence-setup`、一般ユーザー
  権限) はデバイス選択・確認ダイアログのみを担当し、破壊的操作は一切
  行わない。**候補列挙や予備的除外のためにヘルパーを事前に`sudo`実行する
  ことはしない。** `sudo`によるヘルパー呼び出しは、ユーザーが確認画面を
  完了した後の1回だけに限定する。
...
- `MAJOR:MINOR`の取得方法は、GUI側・ヘルパー側の双方で同一形式の値を
  得るため、原則として `lsblk -dn -o MAJ:MIN -- "$DEVICE"` を用いる。
  GUIが渡した値と、ヘルパーが対象デバイスに対して改めて同じコマンドで
  取得した値とを、単純な文字列として厳密に比較する。GNU `stat`の
  `%t:%T`等の書式は16進表現になる場合があり、`lsblk`の`MAJ:MIN`
  (10進表現) とは直接比較できないため、取得方法が異なる値同士を比較する
  実装にはしない。
...
- `PATH`は固定した安全な値を明示的に設定するか、`parted`/`mkfs.ext4`/
  `partprobe`/`udevadm`/`wipefs`/`blkid`/`mount`/`umount`等の特権コマンドを
  絶対パスで呼び出す。
...
- `MAJOR:MINOR`引数は、`DEVICE`に対して`lsblk -dn -o MAJ:MIN --`で改めて
...
| 21 | persistenceパーティションの作成・再読込・特定に失敗 |
...
- 終了コード10〜16は、いずれも**破壊的操作 (`parted`/`mkfs`) を開始する
  前の拒否**であり、対象ディスクには一切変更を加えない。
- 終了コード20以降は、**破壊的操作を開始した後の失敗**であり、対象
  ディスクが途中状態 (パーティションテーブルのみ作成済み、
  ファイルシステム未作成、`persistence.conf`未作成等) になっている
  可能性があることを明示する。
- 終了コード20以降で失敗した場合、ヘルパーは自動的な再フォーマット・
  再試行・ロールバックを一切行わない。
...
- `SIGINT`・`SIGTERM`・`SIGHUP`を受けて終了した場合の終了コードは
  `128+シグナル番号`とする。GUIはこれらの終了コードを検出した場合、
  「処理が中断されました」という日本語メッセージを表示する。
...
jgmenuの`append.csv`とOpenbox右クリックメニュー (`menu.xml`) の両方には
GUI本体 (`mypocketos-persistence-setup`) の起動項目のみを追加する。
特権ヘルパーをメニューやショートカットから直接実行できるようにはしない。
既存の「電源・セッション」とは別項目として配置する想定 (パーティション
...
テストデータが存在する。**このディスクを空ディスクとして再利用・
再フォーマットするテストは行わない。**
...
未使用の空ディスクを追加接続しただけの構成では、新規作成テストを
実施できない (接続済みのpersistenceパーティションが検出され、新規作成が
拒否されるため)。

**GUIによる新規作成テストには、既存の永続化ディスクが一切接続されて
いない、新規の専用・使い捨てテストVM**を用意して使用する。このVMには
次のみを接続する。

- `MyPocketOS-dev.iso` (共有ISO、読み取りのみ)。
- 内容を失ってよいことを明示的に確認した、新規の空qcow2ディスク。

既存の`mypocketos-test`・`mypocketos-uefi-test`、およびそれらの
永続化ディスクは、この新規作成テストのために変更・再フォーマット・
取り外しのいずれも行わない。

新規の専用・使い捨てテストVMでは、次を確認する。

- 接続された空qcow2ディスク (完全に未使用のwhole disk) に対してGUIから
  作成を行い、「Live永続化基盤」の「動作確認」と同等の結果が得られること。
- 候補一覧にCD-ROM (`sr0`)・Live起動元デバイス・`/home`提供デバイスが
  表示されないこと。
- 署名・マウント・スワップ・holders等いずれかに該当するディスクが候補から
  除外されること。

既存persistence拒否の確認は、上記の使い捨てVMとは別に、従来の
`mypocketos-test` / `mypocketos-uefi-test`を用いて**読み取り確認として**
実施する (対象ディスクの内容・署名を変更する操作は行わない)。

- GUIの候補一覧から、既存の永続領域を持つディスク (`/dev/vda`) が
  除外されること。
- ヘルパーを (GUI経由ではなく) 直接、既存の永続領域を持つディスクに対して
  呼び出した場合でも、最終検証によって拒否されること。
- 上記のいずれの試行によっても、対象ディスクの内容および署名が変更
  されないこと。
