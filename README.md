...
- 子パーティションを1つ以上持つ (既存パーティションテーブルがある)
  ...
  - 全ての子パーティションについて `/sys/class/block/<name>/holders/`
    が空であること。
  - **既知の制限**: ここで確認するのは対象ディスクの直接の子パーティション
    のみである。MBR拡張パーティション配下の論理パーティション (親が
    拡張パーティションであるもの) はこの確認の対象に含まれない。
  - **候補一覧に表示されることは、実際に初期化できることを意味しない。**
    実際の初期化はヘルパーが破壊的操作の直前に独立して再検証し、既存の
    パーティションテーブルや署名が残っている場合は、現行の安全設計
    どおり拒否する (後述「helper側の最終検証」節参照)。
...
- 「GUI側の予備的な候補除外」に列挙した条件のうち対象デバイス自身に
  対応するもの (whole diskであること、基本属性が取得できること、
  マウントなし、スワップでないこと、holdersが空であること、Live起動元/
  `/home`のいずれの親diskとも一致しないこと等) の再検証。
- **パーティションテーブルが一切存在しないこと**(`check_pttype_empty`)。
  GUI側の候補一覧は、USB接続が確認できるディスクに限り既存partitionが
  あっても候補として表示するが (2026-09-06修正、「GUI側の予備的な候補
  除外」節参照)、Mode Aの実際の初期化対象は現行の安全設計どおり
  **完全に未使用のディスクのみ**である。既存partitionを持つディスクを
  GUI候補一覧から選択して確認文字列を入力しても、この時点で拒否される
  (exit 15)。
