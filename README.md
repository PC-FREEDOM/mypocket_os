...
- Calamaresによるインストーラ: 実装済み。Live USBとしての利用に加え、
  必要に応じて内蔵ストレージへ通常インストールできます。初回リリースの
  正式機能です。詳細は「通常インストール (Calamares)」節を参照
...
- jgmenuの検索入力は英字検索のみ動作確認済みです。Fcitx5-Mozcを使用した日本語検索は
  ...
  起動には影響しません。
- Calamaresでの通常インストールは、内蔵ディスクを丸ごと初期化する
  「ディスクの消去」のみを初回版の正式サポート範囲とします。Dual Boot・
  既存OSとの共存インストールは初回版の正式サポート対象外です。詳細は
  「通常インストール (Calamares)」節を参照してください。
...
- **NetworkManager dispatcherによるConky再起動**:
  ...
  再起動時のConky起動オプションは`-p 0 -U`(2026-09-11、Wi-Fi接続時の
  再表示までの空白時間を短縮するため、ログイン時autostartの`-p 3`から
  `-p 1`を経て`-p 0`まで短縮。詳細は`reports/ai-review/`の関連レポート
  参照)。
...
**実機E2E確認済み (2026-09-11)**: 2本指を下へ動かすと表示内容も下へ
動く(意図したNatural Scrollingの挙動)こと、2本指を上へ動かすと表示
内容も上へ動くこと、USBマウスのホイール方向に副作用がないことを、
いずれも実機で確認した。また、Conkyのネットワーク表示・Window Snap
表示(後述)を含むISOでも、タッチパッドのNatural ScrollingとConky側の
機能が互いに副作用なく共存することを実機で確認した。

- `xinput list-props`によるlibinputプロパティの直接確認、トラックポイント
  (pointing stick)搭載機・Bluetoothマウスでの確認は未実施のまま残って
  いる。
...
### タッチパッド設定GUI (2026-09-12)

タップ・2本指スクロール・スクロール方向・ポインタ速度を、GUIから
その場で変更できるMyPocketOS独自の設定アプリを搭載しています。

- **起動方法**: アプリメニュー(jgmenu)の「設定」カテゴリ →
  「タッチパッド設定」。ターミナルから`mypocketos-touchpad-settings`
  を直接実行することもできます。
- **設定できる項目**:
  - タップでクリックする(ON/OFF)
  - 2本指でスクロールする(ON/OFF。デバイスが2本指スクロールに対応
    していない場合は選択不可)
  - スクロール方向(「指の動きと同じ方向」/「指の動きと反対方向」)
  - ポインタ速度(5段階)
- **ボタンの挙動**:
  - 「適用」— 現在の入力値を保存し、その場で反映します。GUIは
    閉じずに表示され続けるため、値を試しながら何度でも調整できます。
  - 「OK」— 保存・反映したうえでGUIを閉じます。
  - 「既定値に戻す」— 保存済みの設定を削除し、MyPocketOSの既定値
    (タップ・2本指スクロール・自然な方向すべてON、標準速度)へ
    その場で戻します。GUIは閉じません。
  - ウィンドウを閉じる/Escapeキー — その時点で未適用の変更は保存・
    反映されません。
- 検出した複数のタッチパッドすべてに同時に設定を適用します
  (USBマウス・トラックポイントには適用されません)。
- ログアウト・再ログイン後も、Openbox起動時に保存済みの設定が自動的に
  再適用されます。
- **Persistenceでの設定保持**: Persistence環境で変更した設定は、
  再起動後も保持されることを実機で確認済みです。
- **実機確認済み (2026-09-12)**: GUIの起動・日本語表示・タップ/
  2本指スクロール/スクロール方向/ポインタ速度それぞれの変更と即時
  反映・既定値への復元・アプリメニューからの起動・「設定」カテゴリへの
  配置・「適用」でGUIを閉じずに繰り返し調整できること・「OK」で保存
  してGUIを閉じること・未適用の変更を保存しないことを、いずれも実機で
  確認しています。
- 複数タッチパッドを搭載した実機での確認、USBマウス接続時の副作用が
  無いことの確認は、今回はまだ実施していません。
...
- **専用ディスクへのインストールを初回版の正式サポート範囲とします。**
  インストール先ディスクを丸ごと初期化する「ディスクの消去」を前提と
  しています。
- **Dual Boot、既存OS(Windows等)との共存インストールは、初回版の
  正式サポート対象外です。** Calamaresの画面上には「既存領域を残して
  インストール」「既存OSを置き換える」「手動パーティション」といった
  選択肢も表示されますが、MyPocketOSではこれらの組み合わせについて
  十分な動作検証を行っていません。動作しないと断定するものでは
  ありませんが、初回版では正式な検証・サポートの対象外として扱います。
  他のOSと共存させたい場合、現時点ではCalamaresの使用をお勧めしません。
- ファームウェアに応じて、UEFI環境ではGPT + EFI System Partition
  (512MiB)、Legacy BIOS環境ではMSDOS/MBRのパーティション構成が自動的に
  作成されます。
...
確認済みです。詳細は`mypocketos-specification-2026-08-31.md` 10節、
および`reports/ai-review/`配下の関連レポートを参照してください。
