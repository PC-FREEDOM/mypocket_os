...
check "relaunches conky with -U (unique) and a shortened startup pause (-p 1, intentionally shorter than autostart's -p 3; see 2026-09-11 comment)" \
	grep -qF 'set -- "$@" conky -p 1 -U' "${DISPATCHER}"
...
check "scenario1: runuser call launches conky -p 1 -U (shortened pause, 2026-09-11)" \
	grep -q 'conky -p 1 -U' "${RUNUSER_CALLS}"
