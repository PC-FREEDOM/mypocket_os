...
2026-09-06、タッチパッド既定動作(9.3節)の実機テスト中、動作が重くなった
後の再起動時に、systemd-journaldの`Failed to send WATCHDOG=1
notification message: Transport endpoint is not connected`という
メッセージが繰り返し表示され、再起動が停止する事象が1回発生した。この
事象とタッチパッド設定(`51-mypocketos-touchpad.conf`)との因果関係は
確認しておらず、現時点では独立した未切り分けの調査項目として記録する
にとどめる。
...
7.3節参照）・Openboxのウィンドウスナップ（PR #46、5.4節参照）は、いずれも
実装済みのためこの候補一覧から除外した。

### 優先度中
- Persistence領域が存在する場合、起動メニューでPersistenceを既定選択にできるか検討

## 5.3 Conkyシステム情報パネル仕様

| 表示項目 | 内容 | 状態 |
|---|---|---|
| システム情報 | ホスト名・カーネル・稼働時間・起動モード・CPU使用率・メモリ・ルートFS使用率(各バー付き) | 実装済み |
| ネットワーク | default route interfaceのDown/Up速度。未接続時は「未接続」表示 | 実装済み |
| ショートカット案内 | 既存アプリ操作5項目 | 実装済み |
| Window Snap案内 | 5.4節のWindow Snap仕様(4方向半分スナップ)の案内表示 | 実装済み |

設計要件:

- ネットワーク速度の取得は、特定インターフェース名をハードコードせず、
  default route interfaceを動的に参照すること。
- ネットワーク監視のために、新規exec/execi・常駐監視プロセス・追加
  パッケージを導入しないこと(2節「軽量性」原則に従う)。
- ネットワーク接続状態の変化に追従してConkyを再起動する場合も、新規
  systemdサービス・cron・常時ポーリングは追加せず、既存の
  NetworkManager dispatcher機構のみで完結させること。
- Conky再起動時も表示に必要な実行時環境(ロケール・実行時ディレクトリ
  等)を引き継ぎ、日本語表示・起動モード表示が再起動前後で変化しない
  こと。
- パネルの表示幅は固定とし、表示値の桁数変化によってウィンドウが
  伸縮しないこと。
...
Super + 矢印キーによるウィンドウスナップは、4方向すべてを「その方向の、
現在のモニタの半分へスナップする」という一貫した操作体系として実装
している。

| キー操作 | 挙動 |
|---|---|
| `Super + Left` | 現在のモニタの左半分へスナップ |
| `Super + Right` | 現在のモニタの右半分へスナップ |
| `Super + Up` | 現在のモニタの上半分へスナップ |
| `Super + Down` | 現在のモニタの下半分へスナップ |

設計方針:

- 4分割スナップ(角へのスナップ)は初回公開版では採用しない。
- 同じキーの連続入力によるトグル動作は採用しない。
- 最大化はWindow Snapとは分離し、タイトルバーのダブルクリックや
  タイトルバーの最大化ボタンなど、Openbox標準の既存操作を利用する。
  `Super+Up`/`Super+Down`をMaximize/Unmaximizeの単独動作として割り
  当てることはしない。

実装:

Openbox標準action(`Unmaximize`→`MoveResizeTo`)のみで実装し、追加
スクリプト・daemonは導入しない。4方向とも、ウィンドウが既に最大化
状態であっても確実に半分サイズへ移行できるよう、先に`Unmaximize`
してから`MoveResizeTo`する。右端基準の`x=-0`・下端基準の`y=-0`は、
Openbox 3.6.1の座標指定仕様(値の先頭が`-`かどうかで基準辺を右端/下端
へ切り替える)に基づく正式な記法である。`<monitor>`は明示指定せず
既定値(ウィンドウが現在あるモニタ)を使う。

実装ファイル: `config/includes.chroot/etc/skel/.config/openbox/rc.xml`

2026-09-12、実機で4方向すべての動作を確認済み(ISO LABEL
`MyPocketOS 20260912-22:50`、詳細は`reports/ai-review/`の関連レポート
参照)。マルチモニタ環境での実機確認は未実施。
...
2026-09-06、branch `feat/touchpad-defaults`(commit
`23c946519b91e141de4bac46e7ef4e7b01a1ccc8`)のStandard版ISO
(`mypocketos-standard-amd64.hybrid.iso`、SHA-256
`07617844477ef74841d201392904b93f2ca60778d8a2e0a0bd5ac9ced704d8c9`)で
実機E2E検証を実施した。1本指タップ(左クリック)・2本指タップ(右クリック)・
2本指スクロール・タップ&ドラッグ・物理クリックがいずれも正常に動作する
ことを確認し、`/proc/bus/input/devices`で対象タッチパッド
(`ETPS/2 Elantech Touchpad`・`ELAN0902:00 04F3:3051 Touchpad`)が
カーネルに認識されていることを確認した。USBマウス接続時の左クリック・
...
7.3節参照）・Openbox `Super + 矢印` ウィンドウスナップ（PR #46、5.4節
参照）は、いずれも実装済みのためこの候補一覧から除外した。

1. Persistence存在時の起動既定選択を検討
